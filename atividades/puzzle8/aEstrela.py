...
class No:
    ...
    @property
    def h(self):
        """ Wrapper para a heurística """
        return self.puzzle.heuristica

    @property
    def f(self):
        """ Retorna o valor da função f(n) = g(n) + h(n) """
...
class Solucionador:
    ...
    def resolver(self):
        """
        Realiza busca e retorna um caminho
...
class Puzzle:
    ...
    def heuristica(self):
        """
        A funcao heuristica retorna a soma das distancias de manhattan 
        de cada numero
        """
        distancia = 0
        for i in range(3):
